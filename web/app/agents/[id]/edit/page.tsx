...
export default async function EditAgentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Make sure the agent's current model is always selectable, even if it's no
  // longer installed in Ollama (otherwise editing would silently drop it).
  const installed = modelsResult.ok ? modelsResult.models.map((m) => m.name) : [];
  const modelMissing = agent.model && !installed.includes(agent.model);

  const update = updateAgentAction.bind(null, agent.id);

  return (
    <div>
      <h1 className="text-xl font-bold mb-6">Edit agent</h1>
      <form action={update} className="space-y-4 max-w-2xl">
        <Field label="Name">
          <input name="name" required defaultValue={agent.name} className="input" placeholder="time-bot" />
        </Field>
        ...
        >
          {modelsResult.ok && modelsResult.models.length > 0 ? (
            <select name="model" required className="input" defaultValue={agent.model}>
              {modelMissing && (
                <option value={agent.model}>{agent.model} (not currently installed)</option>
              )}
            ...
            <input name="model" required defaultValue={agent.model} className="input" placeholder="qwen2.5-coder:3b" />
          )}
        </Field>

        <Field label="System prompt">
          <textarea name="systemPrompt" required rows={4} defaultValue={agent.systemPrompt} className="input" placeholder="You are a concise assistant." />
        </Field>
            ...
            {builtIns.map((t) => (
              <label key={t.name} className="flex items-start gap-2 text-sm">
                <input
                  ...
                  name="builtInToolIds"
                  value={t.name}
                  defaultChecked={agent.builtInToolIds.includes(t.name)}
                  className="mt-1"
                />
                <span>
              ...
              {mcpServers.map((s) => (
                <label key={s.id} className="flex items-center gap-2 text-sm">
                  <input
                    ...
                    name="mcpServerIds"
                    value={s.id}
                    defaultChecked={agent.mcpServerIds.includes(s.id)}
                  />
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Max steps">
            <input name="maxSteps" type="number" defaultValue={agent.maxSteps} min={1} max={50} className="input" />
          </Field>
          <Field label="Temperature">
            <input name="temperature" type="number" defaultValue={agent.temperature} step={0.1} min={0} max={2} className="input" />
          </Field>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <a href={`/agents/${agent.id}`} className="px-4 py-2 text-neutral-400 hover:text-neutral-100">
            Cancel
          </a>
        </div>
      </form>
